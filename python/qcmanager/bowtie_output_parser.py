...
class BowtieOutputDictionary:
    ...
    def parse_output(self):
        ...
        for sample in self.samples:
            ...
            aligned_once = int(re.search(r'(\d+) \(\d+\.\d+%\) aligned concordantly exactly 1 time', output).group(1))
            aligned_more_than_once = int(re.search(r'(\d+) \(\d+\.\d+%\) aligned concordantly >1 times', output).group(1))
            overall_alignment_rate = float(re.search(r'(\d+\.\d+)% overall alignment rate', output).group(1))
