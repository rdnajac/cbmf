...
# This is overkill, but it's correct
export PATH="$SCRIPTS_DIR:$PATH"

export VALID_SPECIES=("human" "mouse")
export VALID_ALIGNERS=("bwa" "hisat2" "bowtie2" "star" "subread-align" "subjunct")
...
cmd_align()
{
	...
	[ "$species" = "" ] && bail "Species (-s) is required for alignment"
	[ "$aligner" = "" ] && bail "Aligner (-a) is required for alignment"
	[ "$input_dir" = "" ] && bail "Input directory (-i) is required"
	[ "$output_dir" = "" ] && bail "Output directory (-o) is required"
